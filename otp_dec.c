...
//send dec to make sure connecting to dec server
  charsWritten = send(socketFD, "dec!", 4 , 0);
  if(charsWritten < 0)
    error("CLINET: ERROR writing to socket");
  if(charsWritten < 4)
    printf("CLIENT: WARNING: Not all data written to socket!\n");
//check if connect to dec server
  char check[2] = "1\0";
...
//sent key
  ...
  memset(keyBuffer, '\0', sizeof(keyBuffer));
//read dec text, loop until read all characters
  int readLen = 0;
  do{
